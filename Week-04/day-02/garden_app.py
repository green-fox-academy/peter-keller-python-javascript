from plants import Tree, Flower

class Garden(object):
    def __init__(self):
        self.plants = []

    def add_plant(self, plant):
        self.plants.append(plant)

    def watering(self, amount):
        print("\nWatering with " + str())
        needs_water = self.get_thirsty_plant_indices()
        #amount_to_a_plant = amount / len(needs_water())
       # for i in needs_water:
        #    self.plants[i].watering(amount_to_a_plant)

    def get_thirsty_plant_indices(self):
        thirst_plants_indices = []
        for i in range(len(self.plants)):
            if self.plants[i].is_need_water():
                thirst_plants_indices.append(i)
            return thirst_plants_indices

    def status(self):
        for plant in self.plants:
            plant.status()

my_garden = Garden()
my_garden.add_plant(Flower("yellow"))
my_garden.add_plant(Flower("blue"))
my_garden.add_plant(Tree("purple"))
my_garden.add_plant(Tree("orange"))
my_garden.status()